...
void windowform(){
    ...
    outtextxy(1400,950,"Arya Bhatt & Rajeev Chaudhary");
...
int drawnow(int xcl, int ycl,char cmd,int clr){
    ...
    if(cmd=='e'){
        setcolor(9);
        outtextxy(200,200,"exit clciked will exit in 5 seconds");
        delay(5000);
        exit(1);
    }
    else if(cmd=='p'){
        ...
        int nx,ny;
        outtextxy(110,40,"Enter end point of line in x, y ");
        scanf("%d %d",&nx,&ny);
        nx=nx+xcl;ny=ny+ycl;
        line(xcl,ycl,nx,ny);
        bar(100,30,500,80);
...
void mouseclick(){
    ...
    while(1){ //starts infinite loop
        ...
        if(ismouseclick(WM_LBUTTONDOWN)){ //Detects Mouse Click
            ...
            clearmouseclick(WM_LBUTTONDOWN);//Start assocoating an command to respective click on window
            if(xcl<80 && xcl>5){
                if(ycl<45 && ycl>20){
                    cmd='e';
...
int main(){
cout<<"Input Datas:\n";
initwindow(1500, 1000,"Simple Paint Program using Cpp ",20,20);
windowform();
mouseclick();
}